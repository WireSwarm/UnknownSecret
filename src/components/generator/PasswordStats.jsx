...
export function PasswordStats({ password, isOpen, enableEmojiStats = true, isPostQuantum = false, onFixBcrypt, hasUnicodeCompat = false, onGoToCompatCheck }) {
    ...
    const statItems = [
        ...
        },
        {
            ...
            icon: Globe,
            hideCount: true,
            isCaution: hasUnicodeCompat && !!password,
            onClick: () => setShowCompatInfo(prev => !prev),
            isInteractive: true
        }
    ];

    // Helper for staggered animations in the zxcvbn section
    const getSlideInStyle = (delay) => ({
        opacity: isOpen ? 1 : 0,
        transform: isOpen ? 'translateY(0)' : 'translateY(10px)',
        transition: `all 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) ${delay}ms`,
        animation: `spectacularEntrance 0.8s cubic-bezier(0.22, 1, 0.36, 1) ${delay}ms backwards`
    });

    const renderStatCard = (item, index, alwaysVisible = false) => {
        ...
        const cardStyle = {
            ...
            opacity: alwaysVisible ? 1 : (isOpen ? 1 : 0),
            transform: alwaysVisible ? 'translateY(0)' : (isOpen ? 'translateY(0)' : 'translateY(1rem)'),
            transition: alwaysVisible
                ? 'background 0.3s ease, border 0.3s ease, color 0.3s ease, box-shadow 0.3s ease'
                : `all 0.5s ease ${index * 50}ms`,
    ...
    return (
        <div id="password-stats-outer">
            ...
            {/*
                Single unified flex row.
                Items 0-4: collapse via maxWidth + marginRight when stats are closed.
                Items 5-6 (Bcrypt, Compatibility): always visible, always take their share.
            */}
            <div
                id="password-stats-row"
                style={{
                    display: 'flex',
                    flexDirection: 'column',
                    gap: '0.5rem',
                    marginTop: isOpen ? '1.5rem' : '0.75rem',
                    marginBottom: '0.5rem',
                    transition: 'margin-top 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)'
                }}
            >
                {/* Line 1: collapsible stats (items 0-4) */}
                <div
                    id="password-stats-row-line1"
                    style={{
                        display: 'grid',
                ...
                >
                    {statItems.slice(0, 5).map((item, i) => (
                        <div key={item.label} style={{ minWidth: 0 }}>
                            {renderStatCard(item, i, false)}
                        </div>
                    ))}
                </div>

                {/* Line 2: always-visible stats (Bcrypt, Compatibility) */}
                <div
                    id="password-stats-row-line2"
                    style={{
                        display: 'grid',
                ...
                >
                    {statItems.slice(5).map((item, i) => (
                        <div key={item.label} style={{ minWidth: 0 }}>
                            {renderStatCard(item, 5 + i, true)}
                        </div>
                    ))}
                </div>
            </div>

            {/* Info panels: always accessible regardless of stats open/closed */}
            {showBcryptInfo && (
                <div
                    id="bcrypt-info-panel"
                    style={{
                        marginBottom: '0.5rem',
                        padding: '0.75rem',
                        borderRadius: '0.5rem',
            ...
            {showBcryptInfo && stats.bytes > 72 && (
                ...
                >
                    ...
                    <button
                        id="bcrypt-fix-btn"
                        onClick={() => { if (onFixBcrypt) onFixBcrypt(); setShowBcryptInfo(false); }}
                        style={{
            ...
            {showCompatInfo && hasUnicodeCompat && (
                <div
                    id="compat-warning-container"
                    style={{
                        marginBottom: '0.5rem',
                        padding: '0.65rem 0.75rem',
                        background: 'rgba(234, 179, 8, 0.1)',
                ...
                >
                    ...
                    {onGoToCompatCheck && (
                        <button
                            id="compat-goto-btn"
                            onClick={() => { onGoToCompatCheck(); setShowCompatInfo(false); }}
                            style={{
                                alignSelf: 'flex-start',
                                marginTop: '0.25rem',
                                background: 'rgba(254, 249, 195, 0.15)',
            ...
            {/* Collapsible zxcvbn section */}
            <div
                id="password-stats-container"
                style={{
                    display: 'grid',
